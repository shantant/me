/*!
 * main.js v25.12.6 - Support for Start Page (Major Update)
...
(function ($) {
	...
	$main_articles.each(function () {
		...
		$('<div class="close"></div>')
			...
			.on('click', function (e) {
				e.preventDefault();
				var currentId = $main_articles.filter('.active').attr('id');
				const backToMainIds = ['myjourney', 'itsme', 'myposts'];
				if (backToMainIds.includes(currentId)) {
					location.hash = '';
...
async function reloadpost(data) {
	...
	elemenBaru.innerHTML = `
            <div class="post-full-container">
				...
				<div class="container-post-page">
					<img class="icon-post-page" src="${postDetail.icon}">
	...
	elemenInduk.appendChild(elemenBaru);
	...
	posts.forEach(post => {
		const cardHtml = `
            <div class="cardpost" onclick=reloadpost(${post.id})>
                ...
                <img class="icon_post" src="${post.icon}">
                <h3 class="cardpost__title">${post.judul}</h3>
                <div class="cardpost__description" data-time="${(post.waktu)}">${formatTimeAgoEnglish(post.waktu)}</div>
...
function showNotification() {
	...
	setTimeout(() => {
		...
		setTimeout(() => {
			notification.classList.remove('hide');
		}, 500); // Adjust to transition duration with CSS (0.5s)
